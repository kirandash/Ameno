...
function k_ameno_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && k_ameno_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'k-ameno' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'k-ameno' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'k-ameno' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
